...
class SimpleFrame(ScriptedLoadableModule):
    ...
    def __init__(self, parent):
        ...
        self.parent.title = "Simple Frame"
...
class SimpleFrameWidget(ScriptedLoadableModuleWidget):
    """Uses ScriptedLoadableModuleWidget base class, available at:
    https://github.com/Slicer/Slicer/blob/master/Base/Python/slicer/ScriptedLoadableModule.py
    """

    def setup(self):
        super(SimpleFrameWidget, self).setup()
        ...
        mrmlPath = slicer.modules.simpleframe.path
        mrmlPath = (
            mrmlPath[: mrmlPath.rfind("/")] + "/Resources/Models/SimpleScene.mrml"
        )
        slicer.util.loadScene(mrmlPath)

        # TODO: Refine this section
        hidden_nodes = slicer.util.getNodes(pattern="*NCollar*")

        hidden_nodes["Target"] = slicer.util.getNode("Target")
        hidden_nodes["Frame_Template"] = slicer.util.getNode("Frame_Template")
        hidden_nodes["Frame_Template_wf"] = slicer.util.getNode("Frame_Template_wf")
        ...
        atlasregistrationFormLayout.addRow(
            "Atlas AC/PC/Sup1/Sup2 Fiducials", self.AtlasFids
        )
        ...
        atlasregistrationFormLayout.addRow(
            "Scan AC/PC/Sup1/Sup2 Fiducials", self.ScanFids
        )
        ...
        # Arc Angle value
        #
        self.arc_angle_widget = ctk.ctkSliderWidget()
        self.arc_angle_widget.singleStep = 0.1
        self.arc_angle_widget.minimum = 0
        self.arc_angle_widget.maximum = 180
        self.arc_angle_widget.value = 90
        self.arc_angle_widget.setToolTip("Set Arc angle of approach.")
        parametersFormLayout.addRow("Arc", self.arc_angle_widget)
        ...
        parametersFormLayout.addRow("Collar", self.collar_angle_widget)

        #
        # X-Origin
        #
        self.x_origin_widget = ctk.ctkDoubleSpinBox()
        self.x_origin_widget.singleStep = 0.1
        self.x_origin_widget.minimum = 50
        self.x_origin_widget.maximum = 150
        self.x_origin_widget.value = 100
        self.x_origin_widget.setToolTip(
            "Set threshold value for computing the output image. "
            "Voxels that have intensities lower than this value will set to zero."
        )
        parametersFormLayout.addRow("X0", self.x_origin_widget)

        #
        # Y-Origin
        #
        self.y_origin_widget = ctk.ctkDoubleSpinBox()
        self.y_origin_widget.singleStep = 0.1
        self.y_origin_widget.minimum = 50
        self.y_origin_widget.maximum = 150
        self.y_origin_widget.value = 100
        self.y_origin_widget.setToolTip(
            "Set threshold value for computing the output image. "
            "Voxels that have intensities lower than this value will set to zero."
        )
        parametersFormLayout.addRow("Y0", self.y_origin_widget)

        #
        # Z-Origin
        #
        self.z_origin_widget = ctk.ctkDoubleSpinBox()
        self.z_origin_widget.singleStep = 0.1
        self.z_origin_widget.minimum = 50
        self.z_origin_widget.maximum = 150
        self.z_origin_widget.value = 100
        self.z_origin_widget.setToolTip(
            "Set threshold value for computing the output image. "
            "Voxels that have intensities lower than this value will set to zero."
        )
        parametersFormLayout.addRow("Z0", self.z_origin_widget)
        ...
        self.inputVolumeSelector.connect(
            "currentNodeChanged(vtkMRMLNode*)", self.onRegSelect
        )
        self.inputFiducialSelector.connect(
            "currentNodeChanged(vtkMRMLNode*)", self.onRegSelect
        )
        ...
        self.arc_angle_widget.connect("valueChanged(double)", self.onChanges)
        self.collar_angle_widget.connect("valueChanged(double)", self.onChanges)

        self.x_origin_widget.connect("valueChanged(double)", self.onChanges)
        self.y_origin_widget.connect("valueChanged(double)", self.onChanges)
        self.z_origin_widget.connect("valueChanged(double)", self.onChanges)
        ...
        # path=slicer.util.getModule('PigF-rame').path
        # print path
        ##path=path[:path.rfind('/')]+'Resources/Models/NewPigFr-ameRed.mrml'
        # print path
    ...
    def onApplyRegButton(self):
        ...
        logic.run(
            inputFiducials, inputModel, outputTransform, 0, 100
        )  # Rigid, 100 iterations
    ...
    def RegisterACPC(self):
        ...
        transScale.Translate(
            -pts1[0, 0], -pts1[0, 1], -pts1[0, 2]
    ...
    def pointMoved(self, caller, event):
        # Moves the currently selected Electrode with respects to the position of first
        # Fiducial in the list
        ...
        self.updateCoordsFromFids(moving_point_indx)
    ...
    def updateCoordsFromFids(self, moving_point_indx):
        Entry = [0.0, 0.0, 0.0]
        Target = [0.0, 0.0, 0.0]
        ...
        self.x_origin_widget.setValue(-1 * Target[0])
        self.y_origin_widget.setValue(Target[1])
        self.z_origin_widget.setValue(-1 * Target[2])
    ...
    def onSelect(self):
        ...
        if (
            (selElectrode != self.currElectrode)
            & (selElectrode != None)
            & (self.currElectrode != None)
        ):
            ...
            self.x_origin_widget.setValue(x0)
            self.y_origin_widget.setValue(y0)
            self.z_origin_widget.setValue(z0)
            self.targs.SetNthFiducialPosition(0, -1 * x0, y0, -1 * z0)
            self.arc_angle_widget.setValue(float(selElectrode.GetAttribute("Arc")))
            self.collar_angle_widget.setValue(
                float(selElectrode.GetAttribute("Collar"))
            )
            selElectrode.SetAndObserveTransformNodeID(
    ...
    def onApplyButton(self):
        logic = SimpleFrameLogic()
        arcAngle = -1 * (self.arc_angle_widget.value - 90)
        collarAngle = self.collar_angle_widget.value - 90
        x0 = self.x_origin_widget.value
        y0 = self.y_origin_widget.value
        z0 = self.z_origin_widget.value
    ...
    def newElectrode(self):
        ...
        # The "Logic" has yet to be constructed...along with the unit tests for the SimpleFrame
...
class SimpleFrameLogic(ScriptedLoadableModuleLogic):
    ...
    def run(self, inputVolume, outputVolume, imageThreshold, enableScreenshots=0):
        ...
        if enableScreenshots:
            self.takeScreenshot("SimpleFrameTest-Start", "MyScreenshot", -1)
...
class SimpleFrameTest(ScriptedLoadableModuleTest):
    """
    ...
    def runTest(self):
        ...
        self.setUp()
        self.test_SimpleFrame1()

    def test_SimpleFrame1(self):
